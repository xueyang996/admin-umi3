...
import { Chart, Geom, Axis, Tooltip, Coordinate, Legend, Line, Annotation } from 'bizcharts';

import { Empty } from 'antd';

import { defaultColors, linearDefaultColors, linear90DefaultColors } from '../chartColor';
...
const BasicColumn: FC<BasicChartT> = (props) => {
  ...
  if (useHtml) {
    tooltipPrps.useHtml = useHtml;
  }
  if (htmlContent) {
    tooltipPrps.htmlContent = htmlContent;
  }
  if (tooltipTitle) {
    tooltipPrps.title = tooltipTitle;
  }
  ...
  // 设置横纵坐标 默认字体样式
  const labelXResult = labelX;
  const labelYResult = labelY;
  if (labelXResult && !labelXResult.style) {
    labelXResult.style = {
  ...
  if (labelYResult && !labelYResult.style) {
    labelYResult.style = {
      textAlign: transpose ? 'end' : 'center',
      fill: '#5c6f88', // 文本的颜色
      fontSize: '12', // 文本大小
  ...
  return (
    <Fragment>
      <div
        style={{
          width: `${100 - (emptyFlag ? 0 : selfLegend || 0)}%`,
